// ---------------------------------------------------------------------------
// End-to-end tests for the `baka module create <name>` CLI.
//
...
describe("baka module create — fast (fake LLM, real CLI binary)", () => {
	...
	test("the user can advance through DISCOVER -> DEFINE -> DEVELOP with the proposed action roster visible", async () => {
		...
		try {
			...
			// The user saw the phase progression.
			expect(stdout).toContain("[phase: DISCOVER]")
			expect(stdout).toContain("[phase: DEFINE]")
			expect(stdout).toContain("[phase: DEVELOP]")
			// The user saw the action proposal rendered.
	...
	test("if the bootstrap LLM call fails, the user sees a clear error instead of a silent bare prompt", async () => {
		...
		try {
			...
			// AND the user still sees the context (phase, help) so they
...
describeIfReal("baka module create — slow (real LLM, real CLI binary)", () => {
	...
	beforeAll(async () => {
		...
		// Probe the LLM so the test fails fast with a clear message
		// instead of hanging.
		...
		// Warm the model: a local llama-server may need to load the
		// model on the first request. Issue a tiny completion so the
		// first user-facing call is fast.
		execSync(
	...
	test("the user always sees context before being asked to type", async () => {
		...
		const responsesFile = join(tmpDir, "responses.txt")
		// Pre-scripted answers that should be enough to advance through
		// at least one full round of DISCOVER questions.
		writeFileSync(
			responsesFile,
		...
		// Use module name "nextjs" not "nextjs" — "testmod" is reserved
		// for the fast flavor. The real LLM test should be visibly
		// different.
		const cli = join(__dirname, "..", "dist", "index.js")
		...
		// 2. The phase context was shown.
		expect(out).toMatch(/\[phase: \w+\]/)

		// 3. The slash command help block was rendered on the first prompt.
		expect(out).toContain("/exit")
		expect(out).toContain("/show prefs")
		expect(out).toContain("/back")

		// 4. The session was saved when the user typed /exit.
		expect(out).toMatch(/session saved to/)

		// 5. The user typed a first response. We assert that AT LEAST
		//    SOME LLM output appeared before the first `> ` prompt —
		//    this is the structural property that proves the bootstrap
		//    LLM call fired and the user wasn't left staring at a bare
		//    prompt.
