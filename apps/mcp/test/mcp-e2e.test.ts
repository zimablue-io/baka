// ---------------------------------------------------------------------------
// Black-box end-to-end tests for the `baka-mcp` binary after the
...
// ---------------------------------------------------------------------------
// VAL-MCP-012 — tools/call baka_plan dry-run with fake LLM (worker role only)
// ---------------------------------------------------------------------------

describe("VAL-MCP-012 tools/call baka_plan dry-run", () => {
	it("returns a SUCCESS or FAILED plan shape (fake LLM, worker role configured)", async () => {
...
// produce the same top-level JSON shape.
//
// Both sides use the same role-keyed config (worker role pointing at the
// fake LLM). The role-keyed shape must round-trip through both surfaces
// and produce the same top-level JSON contract.
// ---------------------------------------------------------------------------
